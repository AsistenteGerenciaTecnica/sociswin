...
dentro de las facturas a proveedores
*/
...
class FacturaCotInm  
{
    ...
    function fetch($id)
    {
        ...
        $sql = "SELECT";
		$sql.= " f.rowid,";
		$sql.= " f.presupuesto_fk,";
		$sql.= " f.pedido_fk,";
		$sql.= " f.cotinm_administracion_tipo,";
		$sql.= " f.cotinm_administracion_valor,";
		$sql.= " f.cotinm_administracion_iva,";
		$sql.= " f.cotinm_gravamen_tipo,";
		$sql.= " f.cotinm_gravamen_valor,";
		$sql.= " f.cotinm_seguros_tipo,";
		$sql.= " f.cotinm_seguros_valor,"; 
		$sql.= " f.cotinm_seguros_iva,";
		$sql.= " f.cotinm_costo_transaccion_valor,";
		$sql.= " f.cotinm_costo_transaccion_tipo";     
	...
	// Actualizar los campos dentro de la factura
    ...
    {		
        ...
        $sql.= " pedido_fk=".(isset($this->pedido_fk)?"'".addslashes($this->pedido_fk)."'":"null").",";
	...
	// Actualizar los productos relacionados con la factura
	function update_products_cotinm($linea_id_eliminar = NULL)
	{
		...
		//Eliminar los productos existentes de la cotización que esten en la factura
		$importe_factura = 0;
		...
		{		
			...
			{
				$cotinm_valor_administracion_aplicar = $importe_factura*$cotinm_valor_administracion_aplicar/100;
			}		
			if($cotinm_valor_administracion_aplicar > 0)
			{
				...
				if($producto_administracion->id > 0)
				{
					...
					// Función de la clase FactureFournisseur para añadir el producto a la factura de proveedor
					$object->addline(
						$desc,								// $desc
						$pu_ht,								// $pu
						$tva_tx,							// $txtva
			...
			{
				$cotinm_valor_seguros_aplicar = $importe_factura*$cotinm_valor_seguros_aplicar/100;
			}
			...
			$importe_descontado = $importe_factura - ($cotinm_valor_seguros_aplicar + $iva_seguros) - ($cotinm_valor_administracion_aplicar + $iva_administracion);
			...
			{
				$cotinm_valor_costo_transaccion_aplicar = $importe_factura*$cotinm_valor_costo_transaccion_aplicar/100;
			}
